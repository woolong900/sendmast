-- The block editor (TemplateEditorPage) renders ONLY from design_json. The
-- abandoned-cart default template was seeded as HTML-only, so it sent fine but
-- showed a blank canvas in the editor. Wrap its body HTML in an easy-email
-- `raw` block (a valid direct child of `page`) so the editor displays/edits it.
-- Built with jsonb_build_object so the HTML needs no JSON escaping; the inner
-- markup goes in via a dollar-quoted literal. Idempotent (keyed on the id).
UPDATE "email_templates"
SET "design_json" = jsonb_build_object(
