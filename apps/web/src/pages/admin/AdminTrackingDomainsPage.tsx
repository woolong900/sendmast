 ...
 *   2. Ensure the active Cloudflare Origin Cert covers the host, replacing the
 *      cert files under DATA_ROOT/caddy-certs if needed.
 *   3. Drop the generated `<domain>.caddy` snippet into
 *      DATA_ROOT/caddy-tracking.d/, then validate and reload Caddy.
 *   4. Probe the tracking URL, then click "添加并启用" here. Status defaults to
 *      active and worker-sender starts using the host within ~30s (pool cache
 *      TTL).
 *
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-blue-100 bg-blue-50/60 p-4 text-sm text-blue-900">
      ...
      <ol className="mt-2 list-decimal space-y-1 pl-5 text-blue-800/90">
        ...
        <li>
          点本页"添加域名" → 填域名 → 复制 Caddy 片段,scp 到服务器
          <span className="break-all font-mono">
            {' '}
            /var/lib/sendmast/caddy-tracking.d/&lt;domain&gt;.caddy
          </span>
          。
        </li>
        <li>
          ...
          <span className="break-all font-mono">
            {' '}
            cd /root/sendmast && docker compose --env-file .env.production -f
            docker/docker-compose.prod.yml exec -T caddy caddy validate --config
            /etc/caddy/Caddyfile
          </span>
          。
        </li>
        <li>
          ...
          <span className="break-all font-mono">
            {' '}
            cd /root/sendmast && docker compose --env-file .env.production -f
            docker/docker-compose.prod.yml exec -T caddy caddy reload --config /etc/caddy/Caddyfile
          </span>
          。
        </li>
        <li>
          访问 <span className="font-mono">https://&lt;domain&gt;/t/o/_probe.gif</span>, 能拿到 HTTP
          响应(通常 4xx)即表示 TLS 与 Caddy 路由可用。
        </li>
        <li>最后点击"添加并启用";30 秒内 worker-sender 会把新域名加入轮询池。</li>
      </ol>
    </div>
  );
}
...
# Generated by SendMast admin UI. Drop into /var/lib/sendmast/caddy-tracking.d/
# on the server, then validate and reload Caddy from /root/sendmast.
${domain} {
